...
// import cadenaLogo from './assets/cadenaLogo.p
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const completions = await apiClient.createCompletion({
        ...
        prompt: `Make an instagram caption about: ${prompt} and add emojis and hashtags to make it more fun.`,
        max_tokens: 880,
        temperature: 0.7,
        top_p: 1,
        frequency_penalty: 0,
...
{/*       <img src={cadenaLogo} alt="Cadena Logo" className="h-12 mb-10 mt-40" />*/}
      <h1 className="text-white text-4xl font-bold py-10">Captions_GPT</h1>
      <form onSubmit={handleSubmit} className="w-full max-w-lg">
        <div className="flex items-center border-b-2 border-indigo-600 py-2">
          <input
            className="appearance-none bg-transparent border-none w-full text-white mr-3 py-1 px-2 leading-tight focus:outline-none"
            type="text"
            placeholder="Enter a topic you want captions for..."
            value={prompt}
